...
pub fn getRotationMatrix(self: *@This()) nz.Mat4x4(f32) {
    ...
    const pitchEuler = pitchRotation.toVec();
    const yawEuler = yawRotation.toVec();

    return nz.Mat4x4(f32).fromQuaternion(yawEuler).mul(nz.Mat4x4(f32).fromQuaternion(pitchEuler));
}
